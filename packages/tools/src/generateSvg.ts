...
export async function generate(
  ...
  const getNamesImpl = getNamesFn || getNames;
...
export async function generateReactSVGs(
  ...
  const directoryContents = fsNative.readdirSync(inputDir);

  // Filter out only the SVG files
  const svgFiles = directoryContents.filter((filename) => {
    const fileExtension = path.extname(filename);
    return fileExtension === ".svg";
  });

  // Construct the full path for each SVG file
  const filelist = svgFiles.map((filename) => {
  ...
  await Promise.all(
    filelist.map(async (filePath) => {
      ...
      try {
        ...
        if (res?.status === GeneratorStatus.Skipped) {
          spinner.info(
            `${res.outputFilePath} was skipped as it already exists`,
          );
        }
        if (res?.status === GeneratorStatus.Done) {
